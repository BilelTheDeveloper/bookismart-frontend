...
const CreativeMediaTheme = ({ data }) => {
  if (!data) return null;
  const { ownerId, hero, services, gallery } = data;
  const activeServices = (services || []).filter((s) => s?.active !== false);
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100">
      <section className="relative h-[70vh] bg-cover bg-center" style={{ backgroundImage: `url(${hero?.backgroundImage || "https://images.unsplash.com/photo-1492691527719-9d1e07e534b4?q=80&w=2070&auto=format&fit=crop"})` }}>
        ...
        <div className="relative max-w-6xl mx-auto px-6 h-full flex flex-col justify-center">
          ...
          <h1 className="mt-5 text-5xl md:text-7xl font-black">{hero?.title || "Create. Capture. Publish."}</h1>
      ...
      <section className="max-w-6xl mx-auto px-6 pb-16 grid grid-cols-2 md:grid-cols-4 gap-4">{(gallery?.images || []).filter(Boolean).map((img, i) => <img key={i} src={img} alt="gallery" className="h-40 w-full object-cover rounded-xl border border-white/10" />)}</section>
    </div>
  );
};
