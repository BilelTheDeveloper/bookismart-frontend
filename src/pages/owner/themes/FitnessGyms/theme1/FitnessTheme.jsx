...
const FitnessTheme = ({ data }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100">
      <section className="relative h-[80vh] bg-cover bg-center" style={{ backgroundImage: `url(${hero?.backgroundImage || "https://images.unsplash.com/photo-1517836357463-d25dfeac3438?q=80&w=2070&auto=format&fit=crop"})` }}>
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 h-full flex flex-col justify-center">
          ...
          <h1 className="mt-5 text-4xl sm:text-5xl md:text-7xl font-black">{hero?.title || "Train With Purpose"}</h1>
          <p className="mt-4 max-w-2xl text-slate-200/90 text-base sm:text-lg">{hero?.slogan || "Performance-driven coaching and classes for every level."}</p>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 py-12 grid md:grid-cols-2 gap-5">
      ...
      {gallery?.show && images.length > 0 && <section className="max-w-7xl mx-auto px-4 sm:px-6 pb-12 grid grid-cols-2 md:grid-cols-4 gap-4">{images.map((img, i) => <img key={i} src={img} alt="gallery" className="h-36 sm:h-44 w-full object-cover rounded-xl border border-white/10" />)}</section>}
      ...
      <footer className="max-w-7xl mx-auto px-4 sm:px-6 py-10 border-t border-white/10 grid md:grid-cols-2 gap-4 text-sm">
        ...
        <div className="md:col-span-2">{(businessHours || []).map((h) => <p key={h.day} className="text-slate-300">{h.day}: {h.isClosed ? "Closed" : `${h.open} - ${h.close}`}</p>)}</div>
