...
import {
  ...
  Power, FileText, Star, Briefcase, Globe, ChevronDown, Zap,
  Bell, ShieldCheck, HelpCircle, ExternalLink, MessageSquare
} from "lucide-react";
...
const Sidebar = ({ isCollapsed, setIsCollapsed }) => {
  ...
  const user = JSON.parse(localStorage.getItem("user")) || { fullName: "Owner", role: "owner" };
      ...
      <nav className="flex-1 px-2 overflow-y-auto overflow-x-hidden" style={{ scrollbarWidth: "none" }}>
        {NAV_SECTIONS.map((section, si) => (
          <div key={section.label} className={si > 0 ? "mt-4" : ""}>
            {/* Section label */}
            {!isCollapsed && (
              <p className="px-3 text-[9px] font-black text-slate-600 uppercase tracking-[0.22em] mb-1.5">
                {section.label}
              </p>
            )}
                ...
                return (
                  ...
                  >
                    ...
                    >
                      ...
                      {/* Label */}
                      {!isCollapsed && (
                        <span className="text-sm whitespace-nowrap leading-none">{item.name}</span>
                      )}
            ...
            <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-slate-800/50 border border-slate-700/40 mb-2">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-slate-500 text-[10px] truncate mt-0.5">
                  {user.businessCategory || user.email || "Business Owner"}
                </p>
              </div>
              <Link
