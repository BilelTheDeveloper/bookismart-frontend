...
const KIND_META = {
  package:    { label: "Package",    icon: Package, accent: "indigo", hint: "Prepaid sessions" },
  membership: { label: "Membership", icon: Crown,   accent: "violet", hint: "Time-based access" },
  giftcard:   { label: "Gift Card",  icon: Gift,    accent: "rose",   hint: "Stored value" },
};
const ACCENT = {
...
const Packages = () => {
  ...
  const saveOffer = async () => {
    const o = offerModal;
    if (!o.name?.trim()) { toast.error("Name is required."); return; }
    setBusy(true);
    try {
      if (o._id) await API.put(`/merchant/packages/offers/${o._id}`, o);
      else await API.post("/merchant/packages/offers", o);
      setOfferModal(null); toast.success("Saved"); load();
    } catch (e) { toast.error(e.response?.data?.message || "Save failed"); }
    finally { setBusy(false); }
  };
  const deleteOffer = async (id) => {
    if (!window.confirm("Delete this offer?")) return;
    try { await API.delete(`/merchant/packages/offers/${id}`); load(); } catch { toast.error("Delete failed"); }
  };
  const issue = async (form) => {
    setBusy(true);
    try {
      await API.post("/merchant/packages/issue", { offerId: issueModal._id, ...form });
      setIssueModal(null); toast.success("Issued to customer"); setTab("issued"); load();
    } catch (e) { toast.error(e.response?.data?.message || "Could not issue"); }
    finally { setBusy(false); }
  };
  const redeem = async (cp) => {
    let amount = 1;
    if (cp.kind === "giftcard") {
      const v = window.prompt(`Redeem amount (balance ${cp.balance} TND):`, "");
      if (v == null) return; amount = Number(v);
    }
    try { await API.post(`/merchant/packages/issued/${cp._id}/redeem`, { amount }); toast.success("Redeemed"); load(); }
    catch (e) { toast.error(e.response?.data?.message || "Redeem failed"); }
  };
  const cancelIssued = async (id) => {
    if (!window.confirm("Cancel this record?")) return;
    try { await API.delete(`/merchant/packages/issued/${id}`); load(); } catch { toast.error("Failed"); }
  };

  const issuedShown = issuedFilter ? issued.filter((i) => i.kind === issuedFilter) : issued;

  const kpis = [
    { label: "Active Offers", value: stats.activeOffers, icon: Ticket, accent: "indigo" },
    { label: "Active Sold", value: stats.activeIssued, icon: Users, accent: "violet" },
    { label: "Gift Balance Out", value: `${(stats.giftOutstanding || 0).toFixed(0)} TND`, icon: Gift, accent: "rose" },
    { label: "Package Revenue", value: `${(stats.totalRevenue || 0).toFixed(0)} TND`, icon: DollarSign, accent: "indigo" },
  ];
  ...
  return (
    <div className="space-y-7">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="flex items-center gap-2.5 text-2xl font-black text-slate-900 dark:text-white"><Sparkles className="text-indigo-600 dark:text-indigo-400" /> Packages & Gift Cards</h1>
          <p className="text-slate-500 dark:text-slate-400 font-medium mt-1">Sell prepaid packages, memberships and gift cards — and track redemptions.</p>
        </div>
      ...
      <div className="flex bg-slate-100 dark:bg-slate-800 rounded-2xl p-1 gap-1 w-fit">
        {[["offers", "Offers"], ["issued", "Sold & Issued"]].map(([id, label]) => (
      ...
      {loading ? (
        ...
        offers.length === 0 ? (
          <Empty icon={Ticket} title="No offers yet" sub="Create your first package, membership or gift card." />
        ) : (
              ...
              return (
                <div key={o._id} className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-100 dark:border-slate-800 p-6 shadow-sm flex flex-col">
                  <div className="flex items-start justify-between">
                    ...
                    <span className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase ${a.chip}`}>{meta.label}</span>
                  </div>
                  ...
                  <div className="mt-3 flex items-baseline gap-2">
                    ...
                    <span className="text-xs font-bold text-slate-400">
                      {o.kind === "package" && `· ${o.sessions} sessions`}
                      {o.kind === "membership" && `· ${o.durationDays} days`}
                      {o.kind === "giftcard" && `· ${o.value} TND value`}
                    </span>
                  </div>
                  <div className="mt-5 pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center gap-2">
                    <button onClick={() => setIssueModal(o)} className="flex-1 inline-flex items-center justify-center gap-1.5 rounded-xl bg-slate-900 dark:bg-indigo-600 text-white py-2.5 text-xs font-black hover:bg-indigo-600 dark:hover:bg-indigo-500 transition-all"><Users size={14} /> Issue / Sell</button>
        ...
        <>
          <div className="flex flex-wrap gap-2">
            {[["", "All"], ["package", "Packages"], ["membership", "Memberships"], ["giftcard", "Gift Cards"]].map(([id, label]) => (
          ...
          {issuedShown.length === 0 ? (
            <Empty icon={Users} title="Nothing issued yet" sub="Sell an offer to a customer from the Offers tab." />
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {issuedShown.map((cp) => {
                const meta = KIND_META[cp.kind]; const a = ACCENT[meta.accent]; const Icon = meta.icon;
                const remaining = cp.kind === "package" ? `${cp.sessionsTotal - cp.sessionsUsed}/${cp.sessionsTotal} left`
                  : cp.kind === "giftcard" ? `${cp.balance} / ${cp.value} TND`
                  : cp.expiresAt ? `until ${new Date(cp.expiresAt).toLocaleDateString("en-GB")}` : "";
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <p className="font-black text-slate-900 dark:text-white truncate">{cp.customerName || "Customer"}</p>
                          <span className={`px-2 py-0.5 rounded-full text-[9px] font-black uppercase ${statusColor}`}>{cp.status}</span>
                        </div>
                    ...
                    {cp.status === "active" && (
                      <div className="mt-4 pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center gap-2">
                        <button onClick={() => redeem(cp)} className={`flex-1 inline-flex items-center justify-center gap-1.5 rounded-xl py-2.5 text-xs font-black text-white ${a.text.includes("rose") ? "bg-rose-500 hover:bg-rose-600" : "bg-indigo-600 hover:bg-indigo-500"} transition-all`}>
                          <Check size={14} /> {cp.kind === "package" ? "Use a session" : cp.kind === "giftcard" ? "Redeem" : "Log visit"}
                        </button>
    ...
    <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4" onClick={() => setOffer(null)}>
      <div className="w-full max-w-md rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-5">
          <h3 className="text-lg font-black text-slate-900 dark:text-white">{offer._id ? "Edit offer" : "New offer"}</h3>
            ...
            {Object.entries(KIND_META).map(([k, m]) => {
              ...
              return (
                <button key={k} onClick={() => upd("kind", k)} className={`rounded-xl border py-3 flex flex-col items-center gap-1.5 transition-all ${active ? "border-indigo-500 bg-indigo-50 dark:bg-indigo-500/15" : "border-slate-200 dark:border-slate-700"}`}>
                  ...
                  <span className={`text-[10px] font-black ${active ? "text-indigo-600 dark:text-indigo-400" : "text-slate-500 dark:text-slate-400"}`}>{m.label}</span>
                </button>
              );
            })}
          </div>
          <input value={offer.name} onChange={(e) => upd("name", e.target.value)} className={fieldCls} placeholder="Name (e.g. 10-Session Pack)" />
          <textarea value={offer.description} onChange={(e) => upd("description", e.target.value)} rows={2} className={`${fieldCls} resize-none`} placeholder="Short description" />
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="text-[10px] font-black uppercase text-slate-400">Price (TND)</label>
              <input type="number" value={offer.price} onChange={(e) => upd("price", e.target.value)} className={fieldCls} placeholder="0" />
            </div>
            {offer.kind === "package" && <div><label className="text-[10px] font-black uppercase text-slate-400">Sessions</label><input type="number" value={offer.sessions} onChange={(e) => upd("sessions", e.target.value)} className={fieldCls} /></div>}
            {offer.kind === "membership" && <div><label className="text-[10px] font-black uppercase text-slate-400">Valid (days)</label><input type="number" value={offer.durationDays} onChange={(e) => upd("durationDays", e.target.value)} className={fieldCls} /></div>}
            {offer.kind === "giftcard" && <div><label className="text-[10px] font-black uppercase text-slate-400">Card value (TND)</label><input type="number" value={offer.value} onChange={(e) => upd("value", e.target.value)} className={fieldCls} /></div>}
          </div>
        </div>
        <div className="mt-6 flex gap-3">
          <button onClick={() => setOffer(null)} className="flex-1 rounded-2xl bg-slate-100 dark:bg-slate-800 py-3 font-black text-slate-600 dark:text-slate-300">Cancel</button>
          <button onClick={onSave} disabled={busy} className="flex-[2] flex items-center justify-center gap-2 rounded-2xl bg-indigo-600 py-3 font-black text-white hover:bg-indigo-500 disabled:opacity-60">{busy ? <Loader2 size={16} className="animate-spin" /> : "Save offer"}</button>
        </div>
      </div>
    </div>
  );
    ...
    <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="w-full max-w-md rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-1">
          <h3 className="text-lg font-black text-slate-900 dark:text-white">Issue: {offer.name}</h3>
        ...
        <p className="text-xs text-slate-400 mb-5">{KIND_META[offer.kind].label} · {offer.price} TND</p>
        <div className="space-y-3">
          <input value={form.customerName} onChange={(e) => upd("customerName", e.target.value)} className={fieldCls} placeholder="Customer name" />
          <input value={form.customerPhone} onChange={(e) => upd("customerPhone", e.target.value)} className={fieldCls} placeholder="Phone" />
          <input value={form.customerEmail} onChange={(e) => upd("customerEmail", e.target.value)} className={fieldCls} placeholder="Email (optional)" />
          <div>
            <label className="text-[10px] font-black uppercase text-slate-400">Amount paid (TND)</label>
        ...
        <button onClick={() => onIssue(form)} disabled={busy} className="mt-6 w-full flex items-center justify-center gap-2 rounded-2xl bg-indigo-600 py-3.5 font-black text-white hover:bg-indigo-500 disabled:opacity-60">
          {busy ? <Loader2 size={16} className="animate-spin" /> : <><ChevronRight size={16} /> Confirm & Issue</>}
        </button>
      </div>
    </div>
  );
