...
const SpaWebsite = ({ merchantData }) => {
  ...
  const data = {
    name: merchantData?.name || "Zen Retreat",
    ...
    aboutText: merchantData?.about?.text || "Discover a profound sense of well-being. Our holistic approach combines ancient healing traditions with modern therapeutic techniques to restore balance.",
    ...
    contact: {
      phone: merchantData?.contact?.phone || "+216 71 888 999",
      address: merchantData?.contact?.address || "Gammarth, Tunis",
        ...
        <section id="philosophy" className="py-32 px-6">
          <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-24 items-center">
            ...
            <div className="relative order-1 lg:order-2">
              <div className="absolute -inset-6 border border-teal-100 rounded-full scale-90 md:scale-100"></div>
           ...
           <div className="max-w-7xl mx-auto grid grid-cols-2 lg:grid-cols-3 gap-6">
              {data.gallery.map((img, i) => img && (
                <div key={i} className={`overflow-hidden group relative rounded-2xl shadow-lg ${i === 1 ? 'lg:row-span-2' : ''}`}>
                   ...
                   <div className="absolute inset-0 bg-teal-950/20 opacity-0 group-hover:opacity-100 transition-opacity" />
                </div>
              ))}
           </div>
        </section>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-24 mb-32">
           <div className="space-y-8 text-center lg:text-left">
              <div className="flex items-center justify-center lg:justify-start gap-4">
                 ...
                 <span className="text-3xl font-serif italic">{data.name}</span>
              </div>
              <p className="text-white/40 max-w-xs mx-auto lg:mx-0 font-jost font-light leading-loose">{data.slogan}</p>
              <div className="flex justify-center lg:justify-start gap-6">
                  <a href="#" className="text-white/30 hover:text-teal-400 transition-colors"><InstagramIcon /></a>
                  <a href="#" className="text-white/30 hover:text-teal-400 transition-colors"><FacebookIcon /></a>
              </div>
           </div>
           ...
           <div className="space-y-10 text-center lg:text-left border-y lg:border-y-0 lg:border-x border-white/5 py-12 lg:py-0">
              ...
              <div className="space-y-8">
                 <div className="flex items-center gap-4 justify-center lg:justify-start">
                   ...
                   <p className="text-white/70 font-jost font-light">{data.contact.address}</p>
                 </div>
                 <div className="flex items-center gap-4 justify-center lg:justify-start">
  ...
  return (
    <div className="relative h-full bg-[#fdfcf9]">
      {!isFullscreen && (
